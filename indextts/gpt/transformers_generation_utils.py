...
def construct_attn_mask(phase_1dim_attn_masks, phases_attn_mask_ids, trunc_index, input_attention_masks, dynamic_cond_mask_idx, current_phases, device, mask_dtype):
    causal_masks = []
    for i in range(len(phases_attn_mask_ids)):
        ...
        if input_attention_masks is None:
            ...
            attention_mask_list = [torch.ones((1, output_len), dtype=torch.long, device=device)] * trunc_index
        else:
        ...
        for mask_id in phases_attn_mask_ids[i]:
            attention_mask_list.append(torch.nn.functional.pad(phase_1dim_attn_masks[mask_id], (0, output_len - phase_1dim_attn_masks[mask_id].shape[1]), value=1))
        ...
        if dynamic_cond_mask_idx is not None:
            for j in dynamic_cond_mask_idx:
                attention_mask[0,0,j,:] = torch.nn.functional.pad(phase_1dim_attn_masks[current_phases[i]], (0, output_len - phase_1dim_attn_masks[current_phases[i]].shape[-1]), value=1)
...
class GenerateBeamDecoderOnlyOutput(ModelOutput):
    ...
    hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    past_key_values: Optional[Tuple[Tuple[Tuple[torch.FloatTensor]]]] = None
    attention_mask_ids: Optional[List[List[int]]] = None  # attention-mask id assigned to each token
...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        # Allow the text_positions_mask kwarg to be forwarded.
        if "text_positions_mask" in model_kwargs:
            model_args |= {"text_positions_mask"}
        if "output_path" in model_kwargs:
            model_args |= {"output_path"}
        if "text_last_token_position" in model_kwargs:
            model_args |= {"text_last_token_position"}
        if "method" in model_kwargs:
            model_args |= {"method"}
    ...
    def generate(
        ...
        input_full_attention_mask: Optional[bool] = None,
        input_attention_masks: Optional[torch.Tensor] = None,
        dynamic_cond_mask_idx: Optional[List[int]] = None,
        **kwargs,
    ...
    def heal_tokens(
        ...
        # Many masks ahead may exhaust GPU memory; if memory sharing is on, torch keeps holding RAM instead of freeing VRAM (very slow). Release VRAM here.
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        model_kwargs["attention_masks"] = attention_masks
        model_kwargs["attention_phase"] = [0] * (batch_size * num_beams) # current emotion phase per beam; picks the matching attention mask, 0..len(attention_masks)-1
        emotion_final_phase = len(attention_masks) - 1
        tokenwise_attention_mask = [list() for _ in range(batch_size * num_beams)]  # attention-mask id assigned to each token
        model_kwargs["tokenwise_attention_mask"] = tokenwise_attention_mask
        model_kwargs["input_full_attention_mask"] = input_full_attention_mask
        model_kwargs["input_attention_masks"] = input_attention_masks
        model_kwargs["dynamic_cond_mask_idx"] = dynamic_cond_mask_idx
        self.hmm = None
        
        if model_kwargs.get("method") is not None and model_kwargs.get("method").startswith("max"):
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            have_emotion_twist = self.detect_emotion_twist(
                next_indices,
                next_tokens,
                emotion_final_phase,
                eos_token_id,
            ...
            # if have_emotion_twist:
                # Backoff a few tokens for a smoother emotion transition.
                # backoff_len = min(0, input_ids.shape[-1] - 1)
                # if backoff_len > 0:
                #     input_ids = input_ids[:, :-backoff_len]
                #     cur_len = input_ids.shape[-1]
                #     for i in range(batch_size * num_beams):
                #         tokenwise_attention_mask[i] = tokenwise_attention_mask[i][:-backoff_len]
                # Drop stale cache to keep length in sync with attention masks.
                # model_kwargs["past_key_values"] = None
                # model_kwargs = self._get_initial_cache_position(input_ids, model_kwargs)

                # TODO: roll past_key_values back by one step. Currently breaks hard-mask runs (EOS keeps firing).
                # if model_kwargs.get("past_key_values", None) is not None:
                #     current_kv_cache = model_kwargs["past_key_values"]
                #     new_kv_cache = tuple(
                #         (layer[0][:, :, :-1, :], layer[1][:, :, :-1, :]) 
            ...
            #  ===== Extra bookkeeping for beam reordering =====
            model_kwargs["attention_phase"] = [model_kwargs["attention_phase"][i] for i in cpu_beam_idx]    # critical: keep attention_phase order in sync with the new beams
            # Update tokenwise_attention_mask.
            ...
            # 1. Reorder the HMM state.
            if self.hmm is not None:
                self.hmm.reorder(beam_idx.to(input_ids.device))
            # 2. Reorder segment_positions.
            # segment_positions is a dict {beam_id: [pos_list]}; new beam i comes from old beam beam_idx[i].
            if self.segment_positions is not None:
                new_positions = {}
                for i, old_beam_id in enumerate(cpu_beam_idx):
                    if old_beam_id in self.segment_positions:
                        # List elements are ints, so a shallow copy is enough to avoid aliasing later writes.
                        new_positions[i] = list(self.segment_positions[old_beam_id])
                self.segment_positions = new_positions

            input_ids = torch.cat([input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
            # print("cur_len: ", cur_len, input_ids.shape)
            # print("attention_mask_len: ", [len(x) for x in tokenwise_attention_mask])
            ...
            cur_len = cur_len + 1
            if cur_len >= 1500:
                raise ValueError("Generation exceeded 1000 tokens; stopping forcibly. The model may not have learned eos_token_id; check the training data and pipeline.")
        ...
        tokenwise_attention_mask_ids = []
        generated_len = sequence_outputs["sequences"].shape[-1] - decoder_prompt_len
        for i, beam_id in enumerate(sequence_outputs["beam_indices"]):
            ids = tokenwise_attention_mask[beam_id[0]]
            tokenwise_attention_mask_ids.append(ids[:generated_len])
        return GenerateBeamDecoderOnlyOutput(
            ...
            hidden_states=decoder_hidden_states,
            past_key_values=model_kwargs.get("past_key_values"),
            attention_mask_ids=tokenwise_attention_mask_ids,  # attention-mask id assigned to each token
        )
